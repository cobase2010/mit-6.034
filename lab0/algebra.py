...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1, Product) and isinstance(expr2, Sum):
        vals = []
        for val in expr2:
            vals.append(multiply(expr1, val))
        return Sum(vals)
    if isinstance(expr1, Product) and isinstance(expr2, Product):
        total = 1
        complete_terms = list(expr1) + list(expr2)
        ret_terms = []
        ...
        if len(ret_terms) > 0 and total == 1:
            return Product(ret_terms)
        return Product(ret_terms + [total])
    if isinstance(expr1, Sum) and isinstance(expr2, Product):
        # vals = []
        # for val in expr1:
        #     vals.append(multiply(val, expr2))
        # return Sum(vals)
        return do_multiply(expr2, expr1)   #just apply associative rule here
    if isinstance(expr1, Sum) and isinstance(expr2, Sum):
        vals = []
        for expr1_val in expr1:
            for expr2_val in expr2:
                vals.append(multiply(expr1_val, expr2_val))
        return Sum(vals)
