...
same_identity_rule = IF( OR(AND ('male (?x)', 'male (?x)'), AND ('female (?x)', 'female (?x)')), THEN('same-identity (?x) (?x)'))
brother_rule = IF( AND( AND ('male (?x)', AND ('parent (?z) (?x)', 'parent (?z) (?y)')), NOT ('same-identity (?x) (?y)')), THEN( 'brother (?x) (?y)'))
sister_rule = IF( AND( AND ('female (?x)', AND ('parent (?z) (?x)', 'parent (?z) (?y)')), NOT ('same-identity (?x) (?y)')), THEN( 'sister (?x) (?y)'))
...
sibling_rule = IF( AND( AND('parent (?x) (?y)', 'parent (?x) (?z)'), NOT ('same-identity (?y) (?z)')), THEN( 'sibling (?y) (?z)'))
cousin_rule = IF( AND( AND( AND('parent (?z) (?x)', 'parent (?w) (?y)'), 'sibling (?z) (?w)'), NOT ('same-identity (?x) (?y)')), THEN( 'cousin (?x) (?y)'))
...
# family_rules.
family_rules = [same_identity_rule, brother_rule, sister_rule, sibling_rule, father_rule, mother_rule, son_rule, daughter_rule, cousin_rule, grandparent_rule, grandchild_rule]                    # fill me in
...
# print black_family_cousins, len(black_family_cousins)
...
WHAT_I_FOUND_INTERESTING = 'Figuring out backchaining implementation'
